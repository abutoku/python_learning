...
##--モジュール名
# モジュール名の例
#すべて小文字の短い名前を使います。読みやすくするためにアンダースコアを使っても良い

# my_module

##--パッケージ名
#すべて小文字の短い名前を使います。アンダースコアの利用は推奨されていません。

# mypackage

##--クラス名
#CapWords形式（= 最初大文字でその後はキャメルケース）を用いて、命名
# クラス名の例

# MyClass

##--例外の名前
#例外はクラスであるべきで、名前はCapWords形式を採用すべきです。また、その例外が実際にエラーである場合には、末尾にErrorをつける

# 例外の名前例
#MySomeCaseFooError

##--関数の名前
#すべて小文字を使い、読みやすくするために必要に応じてアンダースコアも利用することができる

# 関数名の例
# my_function

# mixedCaseの命名ルール例 =>camel case
# saySomething

##--関数の引数
#インスタンスメソッドの場合は、必ず最初はself
...
#引数の変数名が予約語と被る場合は、末尾にアンダースコアをつける

# def my_function(self, id_, dict_):
#     pass

#定数
#全て大文字で書き、必要に応じてアンダースコアを利用することもできる
